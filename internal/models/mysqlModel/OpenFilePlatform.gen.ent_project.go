...
func (obj *_EntProjectMgr) WithIsDeleted(isDeleted bool) Option {
...
func (obj *_EntProjectMgr) GetFromIsDeleted(isDeleted bool) (results []*EntProject, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(EntProject{}).Where("`IsDeleted` = ?", isDeleted).Find(&results).Error

	return
}

// GetBatchFromIsDeleted 批量查找 是否删除
func (obj *_EntProjectMgr) GetBatchFromIsDeleted(isDeleteds []bool) (results []*EntProject, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(EntProject{}).Where("`IsDeleted` IN (?)", isDeleteds).Find(&results).Error
