...
func (obj *_EntFileLendMgr) WithIsDeleted(isDeleted bool) Option {
...
func (obj *_EntFileLendMgr) GetFromIsDeleted(isDeleted bool) (results []*EntFileLend, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(EntFileLend{}).Where("`IsDeleted` = ?", isDeleted).Find(&results).Error

	return
}

// GetBatchFromIsDeleted 批量查找 是否删除
func (obj *_EntFileLendMgr) GetBatchFromIsDeleted(isDeleteds []bool) (results []*EntFileLend, err error) {
	err = obj.DB.WithContext(obj.ctx).Model(EntFileLend{}).Where("`IsDeleted` IN (?)", isDeleteds).Find(&results).Error
